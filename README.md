...
| **Daily spend cap**   | `DAILY_GENERATION_LIMIT` env (default 50)                                  | POC demo discipline. Hard cap on GenAI calls per local day. **Server in-memory counter is canonical for enforcement**; browser LocalStorage is a display-only hint for instant paint on page load and is reconciled from the authoritative `{ used, limit, remaining }` block returned by `GET /api/cap` and the terminal NDJSON event of `/api/generate`. UI must never gate the Generate button on LocalStorage alone. |
...
- **Symlinks under `inputs/` and `outputs/` are not followed safely.** `safeJoin` validates that a path is a lexical child of a known root, but does not call `realpath` to re-validate after symlink resolution. Production hardening would add `fs.realpath` re-validation at every boundary that interpolates user-influenced strings (`/api/upload`, `/api/detected-assets`, `/api/reveal`, Sharp reads). Implementers touching those routes in PR #3 should add a `TODO(symlink-hardening)` comment alongside each `safeJoin` call so the gap stays visible. Out of POC scope.
