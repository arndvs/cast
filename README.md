...
| **Daily spend cap**   | `DAILY_GENERATION_LIMIT` env (default 50)                                  | POC demo discipline. Hard cap on GenAI calls per local day. **Source of truth: `outputs/.cap.json` `{ date, count }`** (UTC, atomic tmp-file rename) — survives `next dev` restarts. `GET /api/cap` reads it; the terminal NDJSON event of `/api/generate` returns the post-run `{ used, limit, remaining }`. Browser LocalStorage is an optimistic UI cache only — used for instant first paint, immediately overwritten by the next server response, never authoritative for enforcement. **Server-side enforcement invariant:** the cap is read and incremented at the GenAI call site inside `/api/generate`; the UI must never gate the Generate button on LocalStorage alone, and a client lying about `remaining` cannot bypass enforcement. The cap file lives at `outputs/.cap.json` (one level above any campaign root) so D15's recursive campaign clear cannot wipe it. |
