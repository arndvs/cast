# Flow Diagrams — Cast: Creative Automation Studio Toolchain
...
- `errors[].stage` ∈ `'resolve' | 'genai' | 'resize' | 'compose' | 'compliance' | 'write'` — names the pipeline stage that threw. Closed enum; do not invent new values without updating this table:
  ...
  | `genai`      | OpenAI Images API call failed, or a retryable transport error (e.g. `ETIMEDOUT` / `ECONNRESET`) exhausted retries. Per [D31](#appendix-a--design-decision-register), transient `429`/`5xx` and transport failures retry up to 3 attempts with exponential backoff before this entry is written. `errors[].message` format: HTTP failures → `<status> <provider error string>` verbatim; non-HTTP transport failures → `OpenAI <code>: <message>` (e.g. `OpenAI ETIMEDOUT: ...`). |
...
- Validates each file against the matching sub-schema of `brandProfileSchema` (see [Brand profile schema](#brand-profile-schema-d11--contract)) — `brand.json` against `brandJsonSchema`, `voice.json` against `voiceJsonSchema`, `banned-words.json` (when present) against `bannedWordsSchema`, `logos.json` against `logosManifestSchema`. `font.ttf` is existence-checked only (no parse). Any failure throws `BrandInvalidError` → mapped to `400` with the Zod issue path.
...
| D22 | ~~Daily generation cap~~ — **superseded by [D32](#appendix-a--design-decision-register)** | ~~`DAILY_GENERATION_LIMIT` env (default 50). SoT: `outputs/.cap.json`...~~ Removed in D32: the cap, `outputs/.cap.json`, `GET /api/cap`, the S1 daily-allocation indicator, and the S2′ cap-based Retry gating no longer exist. Original rationale was operator-side spend discipline for a hosted demo; that rationale doesn't apply to a take-home where the reviewer runs locally with their own `OPENAI_API_KEY`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| D29 | Banned-words enforcement parity                                                           | Both client pre-flight (S1) and server-side per-creative compliance call the same `containsBannedWord(text, list)` symbol exported from `lib/banned-words.ts` against the same union list returned by `loadBrandProfile` (lib defaults ∪ brand `banned-words.json`). Route handlers (`/api/brands/[slug]`, `/api/generate`) and the editor client must not re-implement substring matching or list composition. Implementation PR ships a parity test that imports `containsBannedWord` from both `/api/brands/[slug]/route.ts` and `/api/generate/route.ts` and asserts referential identity (same module instance, no copy-paste fork).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| D30 | S2 stream idle timeout                                                                    | The S2 client wraps the `/api/generate` NDJSON `fetch` in an `AbortController` and resets a 90 s idle timer on every received event. After 90 s with no event the client aborts and synthesizes `{ type: 'error', stage: 'stream', message: 'stream idle for 90s — aborted' }` so the state machine transitions `Running → Failed (S2′)`. Prevents an indefinite spinner on a hung GenAI call or mid-stream server crash. 90 s is above worst-case `dall-e-3` latency (≈45 s) but below user-abandonment.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| D31 | GenAI retry semantics; no provider-swap fallback                                          | GenAI calls retry on transient failures (HTTP `429`, `5xx`, network `ETIMEDOUT`/`ECONNRESET`) with bounded exponential backoff: **3 attempts max, delays 1 s → 4 s → 16 s, jittered ±25%**. `Retry-After` (seconds or HTTP-date) is honored when present and overrides the computed delay, capped at 30 s so a single creative cannot block the run past D30's 90 s stream-idle window. Non-transient failures (`4xx` other than `429`, schema/content-policy rejections) do **not** retry. On final exhaustion the per-creative `errors[]` entry preserves the upstream failure verbatim — not a generic "Unexpected response", so S4 error mode surfaces actionable detail. Format: HTTP failures → `<status> <provider error string>` (e.g. `"429 rate_limit_exceeded: ..."`); non-HTTP transport failures → `OpenAI <code>: <message>` (e.g. `"OpenAI ETIMEDOUT: request to /v1/images/generations timed out after 60s"`). **Provider-swap fallback (e.g. OpenAI → Stability/Imagen mid-run) is explicitly out of scope.** Aspect-ratio fidelity (D9: `dall-e-3` renders 3 native sizes with no crop loss) and intra-campaign visual consistency (Story 2: Priya's brand-consistency promise) are higher-priority invariants than provider-availability uptime in a POC. The `CAST_GENAI_MODE` env (`default` ↔ `cheap`) is the operator-time model toggle; runtime model selection is intentionally not a thing. |
| D32 | Drop daily generation cap (supersedes [D22](#appendix-a--design-decision-register))       | The reviewer runs Cast locally against their own `OPENAI_API_KEY`. Capping their spend is paternalistic and adds disproportionate surface area for a take-home POC: a JSON state file with atomic-rename semantics, a runtime `GET /api/cap` endpoint, a server-side enforcement invariant, a LocalStorage cache, an S1 "daily allocation" indicator with client-side projection, and S2′ cap-based Retry gating. **All of it is removed.** D22 is marked superseded (append-only register policy — row not deleted). The `CAST_GENAI_MODE` toggle stays, but the badge now reads `NEXT_PUBLIC_CAST_GENAI_MODE` at build time so no runtime API is needed for it. Spend discipline returns as a real concern only when Cast is hosted on shared infrastructure; v2 reintroduces a cap with a per-account model rather than a global file.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
