...
- **Counts invariant:** `counts.generated + counts.reused === counts.succeeded`. Failed creatives do **not** increment `generated` / `reused` — those buckets are success-only. `flagged` and `failed` are independent axes (a successful creative can be flagged; a failed creative is never flagged because compliance never ran).
...
- `compliance.badge` ∈ `'OK' | 'WARN' | 'FAIL'`. The `compliance` field is **omitted** on any failed creative — compliance only runs after `compose` succeeds, so failures at stages `resolve | genai | resize | compose` cannot carry a compliance result. The grid never renders a compliance badge for a failed tile (red placeholder + stage label takes its place).
