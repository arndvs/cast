...
- `errors[].stage` ∈ `'resolve' | 'genai' | 'resize' | 'compose' | 'compliance' | 'write'` — names the pipeline stage that threw. Closed enum; do not invent new values without updating this table:
  ...
  | `genai`      | OpenAI Images API call failed (after retries per [D31](#appendix-a--design-decision-register) — transient `429`/`5xx` retry up to 3 attempts with exponential backoff before this entry is written), timed out, or hit the daily cap. `errors[].message` carries the upstream HTTP status + provider error string verbatim. |
