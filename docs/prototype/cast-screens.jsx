    ...
    <div className="s1">
      ...
      <div className="s1-sidebar">
        ...
        {brand.products.map((p, i) => {
          const upRaw = uploadedAssets[p.slug];
          const upObj = (upRaw && typeof upRaw === "object") ? upRaw : null;
          const isUploaded = !!upRaw;
          const isPrePlaced = !isUploaded && i !== 0;
          const has = isUploaded || isPrePlaced;
          return (
            <div key={p.slug} className={`detected-row ${has ? "found" : "missing"}`}>
              ...
              {isUploaded && <span className="src-pill src-uploaded" title="dropped via the brief editor">uploaded</span>}
              {isPrePlaced && <span className="src-pill src-preplaced" title="found in inputs/ folder">pre-placed</span>}
              {!has && <span className="src-pill src-genai" title="will be generated by dall-e-3">→ genai</span>}
            </div>
          );
        })}
